...
class OffsetsInitializerUtilsTest {
    ...
    void testParseFormattedTimestamp() {
        // Note: This test might be sensitive to timezone, but for testing purposes we assume system
        // timezone
    ...
    void testParseTimestampInvalidFormats() {
        // Null and empty
        assertThrows(
                IllegalArgumentException.class, () -> OffsetsInitializerUtils.parseTimestamp(null));
        assertThrows(
                IllegalArgumentException.class, () -> OffsetsInitializerUtils.parseTimestamp(""));
        assertThrows(
                IllegalArgumentException.class,
                () -> OffsetsInitializerUtils.parseTimestamp("   "));

        // Invalid numeric
        assertThrows(
                IllegalArgumentException.class, () -> OffsetsInitializerUtils.parseTimestamp("0"));
        assertThrows(
                IllegalArgumentException.class, () -> OffsetsInitializerUtils.parseTimestamp("-1"));
        assertThrows(
                IllegalArgumentException.class,
                () -> OffsetsInitializerUtils.parseTimestamp("abc"));

        // Invalid date format
        assertThrows(
                IllegalArgumentException.class,
                () -> OffsetsInitializerUtils.parseTimestamp("2023-13-45 25:70:80"));
        assertThrows(
                IllegalArgumentException.class,
                () -> OffsetsInitializerUtils.parseTimestamp("2023/03/15 10:30:47"));
        assertThrows(
                IllegalArgumentException.class,
                () -> OffsetsInitializerUtils.parseTimestamp("15-03-2023 10:30:47"));
    }

    @Test
    void testFormatTimestamp() {
        long timestamp =
                1678883047356L; // 2023-03-15 10:30:47.356 (approximately, depends on timezone)
    ...
    void testValidateTimestampForMode() {
        ...
        assertDoesNotThrow(
                () ->
                        OffsetsInitializerUtils.validateTimestampForMode(
                                StartupMode.TIMESTAMP, 1678883047356L));
        ...
        assertDoesNotThrow(
                () -> OffsetsInitializerUtils.validateTimestampForMode(StartupMode.EARLIEST, null));
        assertDoesNotThrow(
                () ->
                        OffsetsInitializerUtils.validateTimestampForMode(
                                StartupMode.EARLIEST, 1678883047356L));
        assertDoesNotThrow(
                () -> OffsetsInitializerUtils.validateTimestampForMode(StartupMode.LATEST, null));
        assertDoesNotThrow(
                () ->
                        OffsetsInitializerUtils.validateTimestampForMode(
                                StartupMode.LATEST, 1678883047356L));
        assertDoesNotThrow(
                () -> OffsetsInitializerUtils.validateTimestampForMode(StartupMode.FULL, null));
        assertDoesNotThrow(
                () ->
                        OffsetsInitializerUtils.validateTimestampForMode(
                                StartupMode.FULL, 1678883047356L));
    }
    ...
    void testCreateFromModeAndTimestamp() {
        ...
        OffsetsInitializer earliest =
                OffsetsInitializerUtils.createFromModeAndTimestamp(StartupMode.EARLIEST, null);
        assertEquals(StartupMode.EARLIEST, earliest.getStartupMode());

        // LATEST mode
        OffsetsInitializer latest =
                OffsetsInitializerUtils.createFromModeAndTimestamp(StartupMode.LATEST, null);
        assertEquals(StartupMode.LATEST, latest.getStartupMode());

        // FULL mode
        OffsetsInitializer full =
                OffsetsInitializerUtils.createFromModeAndTimestamp(StartupMode.FULL, null);
        assertEquals(StartupMode.FULL, full.getStartupMode());

        // TIMESTAMP mode with numeric timestamp
        OffsetsInitializer timestamp1 =
                OffsetsInitializerUtils.createFromModeAndTimestamp(
    ...
    void testCreateFromModeAndTimestampInvalid() {
        ...
        assertThrows(
                ...
                () ->
                        OffsetsInitializerUtils.createFromModeAndTimestamp(
                                StartupMode.TIMESTAMP, "0"));
        assertThrows(
                IllegalArgumentException.class,
                () ->
                        OffsetsInitializerUtils.createFromModeAndTimestamp(
                                StartupMode.TIMESTAMP, "invalid"));
    }
